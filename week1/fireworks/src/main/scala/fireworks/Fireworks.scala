...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      val position2 = Motion.movePoint(position, direction, Settings.propulsionSpeed)
      copy(countDown = countDown - 1, position2)
    else
...
case class Particle(horizontalSpeed: Double, verticalSpeed: Double, position: Point, color: Color):
  ...
  def next: Particle =
    ...
    copy(horizontalSpeed = updatedHorizontalSpeed, verticalSpeed= updatedVerticalSpeed, updatedPosition, color)
